...
non_zero = st.floats(min_value=1e-3, max_value=1.0, allow_infinity=False, allow_nan=False) | st.floats(
    max_value=-1e-3, min_value=-1.0, allow_infinity=False, allow_nan=False
)
...
def test_cram48_vs_pade_on_identity_matrix(cram48_serial: Exponentiator, m: csr_matrix, v: np.ndarray, t: float):
    m = csr_matrix(m)
    z = csr_matrix(m.shape)
    vec = np.array(v)
    cramsol = cram48_serial(m, z, lambda x: 0.0, vec.copy(), t)
    padesol = spla.expm_multiply(m, vec, 0.0, t, num=5, endpoint=True)[-1]
...
def test_cram_is_same_as_magnus_for_constant_flux(
    ...
    n = np.array(n)
    cramsol = cram48_serial(d, r, lambda x: 1.0, n, t)
