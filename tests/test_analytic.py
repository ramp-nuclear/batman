...
def iodine_decay() -> DecayGraph:
    ...
    """
    dec = DecayGraph()
    processes = parse_decay_processes(test_data_dir / 'dec-053_I_135.endf',
                                      test_data_dir / 'dec-054_Xe_135.endf')
    for process in processes:
        target, = process.targets
        if Xe135 in {process.parent, target}:
            process = DecayProcess.from_other(process, fraction=1.)
        dec.add_edge_from_process(process)
    return dec


density_range = st.just(0.) | st.floats(min_value=1e-6, max_value=1e-1)
time_range_seconds = st.floats(min_value=1., max_value=30. * 24. * 3600.)
reac_rates = st.just(0.) | st.floats(min_value=1e-9, max_value=1e-2)
...
def test_analytic_decay_is_exponential_down(
        iodine_decay: DecayGraph,
        t: Second):
    _filter = GraphFilter(whitelist_filter, frozenset({I135}))
    mixture = Mixture({I135: 1.}, 20.)
    rate = iodine_decay[I135][I135][(DECAY,)]['rate']
    expected_res = Mixture({I135: exp(rate * t)}, 20.)
    data = InputData([iodine_decay], [[]], [_filter], [mixture],
                     [1.])
    easy = EasyData.from_input(data)
    config = Configuration(p_rtol=1., p_atol=0, integrator=predictor,
                           time_guesser=tguess, threshold=1e-18)
...
def test_analytic_power_constant_burn_causes_linear_drop_in_fuel(
        fission_reaction: Iterable[ReactionRate],
        t: Second,
        P: MW
):
    acc = 1e-1
    max_step = 2
    _filter = GraphFilter()
    volume = 0.05 * 60 * 6 * 21 * 28.
    rate = P / (U235FISS_ENER * eV * 1e18) / volume
    u0 = 2e-3
    mixture = Mixture({U235: u0}, 20.)
    _densities = {U235: u0 - rate * t, I135: rate * t}
    expected_res = Mixture({k: v for k, v in _densities.items() if v > 1e-18},
                           20.)
    data = InputData([DecayGraph()], [fission_reaction], [_filter], [mixture],
                     [volume])
    easy = EasyData.from_input(data)
    isos, _, r = easy.models[0]
    assert r.energy_model[isos.index(U235)] > 0., r.energy_model
    config = Configuration(p_rtol=acc, p_atol=P * acc, integrator=predictor,
                           time_guesser=tguess, threshold=1e-18)
    if max_step == 1:
    ...
    logging.debug(f'Depletion was: {1. - expected_res[U235] / u0:%}')
    logging.debug(f'It took {result.steps} steps')
    assert allclose(newden, expected_res,
                    comparator=isclose,
...
def test_I135_analytic_solution(iodine_decay, I0, rateI, t):
    ...
    sol = solve_ivp(fun=_deriv_iodine, t_span=(0, t), y0=np.array([I0]),
...
def test_follows_analytic_solution_for_U235_iodine_system(
        iodine_decay, I0, t, rateI):
    _filter = GraphFilter(whitelist_filter, frozenset({I135, U235}))
    mixture = Mixture({I135: I0, U235: 1.}, 20.)
    decay_rate = -iodine_decay[I135][I135][(DECAY,)]['rate']
    Iinf = rateI / decay_rate
    assert Iinf >= 0
    U235nf = ProtoReaction.from_reaction(U235, Fission, branching={I135: 1.},
                                         energy=0.)
    reactions = (ReactionRate('moo', U235nf, rateI, 0.),)
    expected_res = Mixture({I135: _iodine_den(Iinf, I0, decay_rate, t),
                            U235: 1.}, 20.)
    data = InputData([iodine_decay], [reactions], [_filter], [mixture],
                     [1.], accumulates=[frozenset({U235})])
    easy: EasyData = EasyData.from_input(data)
    easy.calc_norm = lambda *a, **ka: 1.
    config = Configuration(p_rtol=1., p_atol=0., integrator=predictor,
                           time_guesser=tguess, threshold=1e-18)
    newden, result = timestep_constant_power(easy, 0, t, config=config)
    assert allclose(newden[0], expected_res)
...
@given(I0=density_range, X0=density_range, t=time_range_seconds,
       rateI=reac_rates, rateX=reac_rates, sigX=reac_rates)
def test_follows_analytic_solution_for_constant_U235_iodine_xenon_system(
        iodine_decay, I0, X0, t, rateI, rateX, sigX):
    _filter = GraphFilter(whitelist_filter, frozenset({I135, U235, Xe135}))
    mixture = Mixture({I135: I0, Xe135: X0, U235: 1.}, 20.)
    decay_I = -iodine_decay[I135][I135][(DECAY,)]['rate']
    decay_X = -iodine_decay[Xe135][Xe135][(DECAY,)]['rate'] + sigX
    cX = rateX
    cI = rateI
    expected_res_x = (X0 * exp(-decay_X * t)
                      + ((cX + cI) / decay_X) * (1 - exp(-decay_X * t))
                      + (((cI - I0 * decay_I) / (decay_X - decay_I))
                         * (exp(-decay_X * t) - exp(-decay_I * t)))
                      )
    Ieq = cI / decay_I
    expected_res_i = _iodine_den(Ieq, I0, decay_I, t)
    bI, bX = ((rateI / (rateI + rateX), rateX / (rateI + rateX)) if (rateI + rateX) > 0.
              else (0., 0.))
    U235nf = ProtoReaction.from_reaction(U235, Fission,
                                         branching={I135: bI, Xe135: bX},
                                         energy=0.)
    Xe135a = Reaction.from_reaction(Xe135, NGamma)
    reactions = tuple(ReactionRate('moo', reac, mean, 0.)
                      for reac, mean in zip([U235nf, Xe135a],
                                            [rateI + rateX, sigX]))
    data = InputData([iodine_decay], [reactions], [_filter], [mixture],
                     [1.], accumulates=[frozenset({U235})])
    easy = EasyData.from_input(data)
    ...
    assert res_mix.get(U235) == pytest.approx(1., rel=1e-10)
...
def numerically_solve_xe_ODE(
        *, U0: float, I0: float, X0: float,
        rateI: float, rateXcreate: float, rateF: float, rateXabs: float,
        decay_X: float, decay_I: float, dt: float,
        rtol: float = 1e-8, atol: float = 1e-14) -> float:
    ...
    def _xe_deriv(t: float, xe: np.ndarray) -> np.ndarray:
        return (decay_I * iodine_density(t=t)
                + rateXcreate
                - (decay_X + _xe_absorption(t=t)) * xe
                )

    sol = solve_ivp(fun=_xe_deriv, t_span=(0., dt), y0=np.array([X0]),
                    rtol=rtol, atol=atol)
    # noinspection PyUnresolvedReferences
    assert sol.status == 0
    # noinspection PyUnresolvedReferences
...
uden_range = st.shared(st.just(0.) | st.floats(min_value=1e-4, max_value=1e-2),
                       key='fuel')
challenge_times = st.floats(min_value=1., max_value=30.)
densities = st.tuples(density_range, density_range, uden_range) \
    .filter(lambda x: sum(x) > 0)
pow_st = st.shared(uden_range.map(lambda x: 20. if x else 0.), key='pow')
vol_st = st.just(60. ** 3)
time_st = st.tuples(uden_range, pow_st, vol_st, challenge_times) \
    .map(lambda x: min(x[-1], _max_burntime(*x[:-1])))
pow_reac_rates = st.tuples(reac_rates, pow_st) \
    .map(lambda x: x[0] if x[1] else 0.)
...
@given(density0=densities, days=time_st, p=pow_st, vol=vol_st,
       rateI=pow_reac_rates, rateX=pow_reac_rates, sigX=reac_rates)
def test_follows_analytic_solution_for_U235_iodine_xenon_system(
    ...
    _filter = GraphFilter(whitelist_filter, frozenset({I135, U235, Xe135}))
    mixture = Mixture({I135: I0, Xe135: X0, U235: U0}, 20.)
    decay_I = -iodine_decay[I135][I135][(DECAY,)]['rate']
    decay_X = -iodine_decay[Xe135][Xe135][(DECAY,)]['rate']
    bI, bX = ((rateI / rateF, rateX / rateF) if rateF > 0.
              else (0., 0.))
    U235nf = ProtoReaction.from_reaction(U235, Fission,
                                         branching={I135: bI, Xe135: bX},
                                         energy=U235FISS_ENER)
    Xe135a = Reaction.from_reaction(Xe135, NGamma)
    reactions = tuple(ReactionRate('moo', reac, mean, 0.)
                      for reac, mean in zip([U235nf, Xe135a],
                                            [rateF / U0 if U0 else 0., sigX]
                                            ))
    expected_res_x = numerically_solve_xe_ODE(
        U0=U0, I0=I0, X0=X0,
        rateI=rateI, rateXcreate=rateX, rateXabs=sigX, rateF=rateF,
        decay_X=decay_X, decay_I=decay_I, dt=t)
    expected_res_i = _iodine_den(Ieq=rateI / decay_I, I0=I0, decay_I=decay_I,
                                 t=t)
    expected_res_u = _uranium_den(U0, rateF, t)
    data = InputData([iodine_decay], [reactions], [_filter], [mixture], [vol])
    easy = EasyData.from_input(data)
    config = Configuration(p_rtol=1e-2, p_atol=1e-10,
