...
class DistEasyData(EasyData):
    ...
    @classmethod
    def from_input(cls: Type[_Tv], data: InputData, *, dtype: str = "float64", partitions: int = 1) -> _Tv:
        ...
        depdict = {i: depargs.index(_get_depargs(comp)) for i, comp in enumerate(data)}

        def expand_bag(*values):
            """Expand a short solution range into the full Bag shape"""
            return from_delayed_sequence(
                delayed(partial(_expand, length=len(data), mp=depdict), pure=True)(values), n=partitions
            )

        make_data_bag = partial(_from_sequence, n=partitions)
        # Lazyily make the minimal amount of depletion models needed
        tiny_models = [delayed(_depletion, nout=3, pure=True)(args, dtype=dtype) for args in depargs]
        isos, decmodels, rg_filters = zip(*tiny_models)
        reacts = tuple(args[1] for args in depargs)
        # Lazily make the fission vectors
        fiss_vecs = [delayed(fiss_arr_gen, pure=True)(isolist, reaclist) for isolist, reaclist in zip(isos, reacts)]
...
ModelInput = Tuple[DecayGraph, Iterable[ReactionType], GraphFilter, FrozenSet[ZAID]]


def _depletion(comp: ModelInput, *, dtype: str) -> DepletionModel:
    dg, rs, filt, accum = comp
    return depletion_model(dg, frozenset(rs), filt, dtype=dtype, accumulate=accum)


def _reacmodel(
    rr: Iterable[ReactionRate],
    *,
    ...
    return ReactionModel(
        isos=isotopes, reactions=reacts, dtype=dtype, accumulate=accumulate, branch_factory=lambda x, _: fiss_vecs[x]
    )
...
def _expand(shortit: Sequence[T], length: int, mp: Dict[int, int]) -> List[T]:
