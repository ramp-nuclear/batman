...
def _time_guess(
    *,
    ...
    fallback: Second = None,
) -> Second:
    xdiff = min(_taylor1_est(x0=x0, y0=y0, y=y, dydx=dydx) - minx, maxx - minx)
    if minx + xdiff in disallowed or xdiff < 0:
        if fallback is not None and fallback not in disallowed:
            ...
            raise ValueError(f"Got an illegal time step of {xdiff:.0f} with no legal fallback: {fallback}")
    else:
        return xdiff


def step_desired_k_at_power(
    data: EasyData,
    *,
    p: MW,
    k0: float,
    ...
    tguess = guess or _time_guess(
        x0=0.0, y0=k0, y=k, dydx=deriv_k(data, loss_factor=loss_fac, p=p), minx=0, maxx=maxt, disallowed=frozenset()
    )
    ...
    modlogger.info("Maximal allowed step reached" if np.isclose(t, maxt) else "k within tolerance")
    k_dot = info.dkdt or deriv_k(data, loss_factor=loss_fac, p=p)
    info = BurnResult(k=info.k, time=info.time, dkdt=k_dot, steps=info.steps, last_norm=info.last_norm)
    return data.mixtures, info


def _try_to_converge(
    data: EasyData,
    t: Second,
    tguess: Second,
    *,
    ...
    newdata.mixtures, info = timestep_constant_power(data, p, tguess, loss_fac=loss_fac, config=config)
    cur_k, k_dot = info.k, info.dkdt
    data = data if too_low(cur_k) else newdata
    t = t if too_low(cur_k) else tguess
    tguess = _time_guess(
        x0=tguess, y0=cur_k, y=ktarget, dydx=k_dot, minx=t, maxx=maxt, fallback=tguess / 2, disallowed=frozenset()
    )
