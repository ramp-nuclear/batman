...
class DecayModel:
    ...
    def __init__(
        self, dg: DecayGraph, isos: Sequence[ZAID], *, dtype: str = "float64", accumulate: FrozenSet[ZAID] = frozenset()
    ):

        n = len(isos)
        self.mat = graph_to_sparse_matrix(dg, isos, dtype=dtype, accumulate=accumulate)
        energy = {
            iso: -rate * e * EV_TO_MJ
            for (iso, _, e), (_, _, rate) in zip(
                nx.selfloop_edges(dg, data="energy", default=0.0), nx.selfloop_edges(dg, data="rate", default=0.0)
            )
        }
        self.energy_model = np.fromiter((energy.get(iso, 0.0) for iso in isos), dtype=dtype, count=n)
